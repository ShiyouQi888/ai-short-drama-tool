...
import { X, Zap, Clock, Upload } from "lucide-react";
...
export function PropertyPanel() {
  const { nodes, selectedNodeId, selectNode, updateNodeData, toggleNodeAsync } =
    usePipelineStore();
    ...
    switch (selectedNode.type) {
      ...
      case "storyboard":
        return (
          <>
            <div className="space-y-2">
              <Label>分镜数量: {selectedNode.data.sceneCount as number}</Label>
              <Slider
                value={[(selectedNode.data.sceneCount as number) || 6]}
                onValueChange={([value]) => updateNodeData(selectedNode.id, { sceneCount: value })}
                min={3}
                max={20}
                step={1}
                className="py-2"
