...
export function PipelineCanvas() {
  ...
  const handleWheel = useCallback(
    (e: WheelEvent) => {
      ...
      const leftZoneWidth = 320; // 导航栏区域宽度 (NodePanel w-72 = 288px + absolute left-4 = 16px, total ~304px)
      const rightZoneWidth = 100; // 右侧感应区宽度
      const windowWidth = canvasRect.width;

      // 1. 左侧导航栏区域：不执行画布操作，允许组件内部滚动
      if (mouseX < leftZoneWidth) {
        // 如果鼠标在左侧面板内，不阻止默认行为，让 NodePanel 内部滚动生效
        return;
      }

      // 只有不在左侧区域时，才阻止默认行为（防止页面滚动）
      e.preventDefault();
      e.stopPropagation();

      // 2. 右侧区域：整个面板上下滚动（垂直平移）
      if (mouseX > windowWidth - rightZoneWidth) {
        pan({
          x: 0,
          y: -e.deltaY,
        });
      ...
      if (newScale !== scale) {
        // 为了实现以鼠标为中心的缩放，我们需要调整 offset
        const mouseXCanvas = (e.clientX - canvasRect.left - offset.x) / scale;
        const mouseYCanvas = (e.clientY - canvasRect.top - offset.y) / scale;
        
        const newOffsetX = e.clientX - canvasRect.left - mouseXCanvas * newScale;
        const newOffsetY = e.clientY - canvasRect.top - mouseYCanvas * newScale;
        
        setScale(newScale);
        pan({
          x: newOffsetX - offset.x,
          y: newOffsetY - offset.y,
        });
      }
    },
    [scale, setScale, pan, offset]
  );
  ...
  const getConnectingLineStart = () => {
    ...
    if (connectingFrom.portType === "output") {
      ...
      const baseY = HEADER_HEIGHT + BODY_PADDING_TOP;
  ...
  return (
    ...
    >
      ...
      {/* Right Scroll Zone Hint */}
      <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-20 z-10 flex items-center justify-end pr-2 opacity-0 hover:opacity-100 transition-opacity">
        <div className="h-1/3 w-1 rounded-full bg-primary/20" />
      </div>
